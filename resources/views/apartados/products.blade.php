            ...
            <td class="td-actions text-right">
                <a href="{{ url('/productos/edit/'. $prod->id ) }}">
                  <button type="button" class="btn btn-success btn-link btn-icon btn-sm" data-toggle="tooltip" data-placement="top" title="Editar Producte"><i class="tim-icons icon-settings"></i></button>
                    ...
                    <button type="submit" class="btn btn-danger btn-link btn-icon btn-sm" style="display:inline" data-toggle="modal" data-placement="top" title="Eliminar Producte" data-target="#{{ str_replace(' ', '', $prod->name) }}">
...
@foreach ($productos as $prod)
  <!-- Modal -->
  <div class="modal fade" id="{{ str_replace(' ', '', $prod->name) }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
